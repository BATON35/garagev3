...
public interface WorkerRepository extends UserRepository<Worker> {
    ...
    @Query(value = "select substring_index(substring_index(j.created_date, ' ', 1), '-', 2) as date, sum(cs.price) as price, w.name from job j join car_service cs on j.car_service_id = cs.id join login.user w on w.id = j.worker_id group by substring_index(substring_index(j.created_date, ' ', 1), '-', 2), w.id", nativeQuery = true)
